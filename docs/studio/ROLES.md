...
- **机器名 → 角色映射**：
  ...
  | 计算机名 | 角色 | 负责范围 |
  |---|---|---|
  | `701-PC`（本机）| **Agent-商城** | `whoknow-mart/` + `whoknow-brain/`（契约中枢）|
  | `DuckyPC` | **Agent-外卖** | `whoknow-waimai/` + 主站前端（`data/` `styles/` `js/` `index*.html`）|

- **仓库只放"地图"，不放"我是谁"**：映射表（本文件）入库共享；"我是谁"的结论缓存到本机 gitignored 的 `.workbuddy/memory/`，不与另一台冲突。
