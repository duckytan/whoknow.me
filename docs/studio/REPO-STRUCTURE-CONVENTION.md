...
| `INDEX.md` | 项目总入口（新 agent onboarding 第一站）| 用户 2026-07-26 裁定：多机流动开发需根级总入口，新 agent 拉取后先读此处；本规范据此增列（详见下方"已整改"）|
...
- ✅ `INDEX.md` 列入根白名单（用户 2026-07-26 裁定：无固定办公地点、多机流动开发，需根级"项目总入口"让全新 agent 拉取后立即投入工作；§2 据此增列；内容含概况/铁律/进度/规范指引/结构/身份确认/协同协议/分工表/onboarding/避坑/术语/导航，细节点链接进对应详文档）。
