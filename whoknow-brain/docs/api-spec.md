...
| 产品 | 当前状态 | brain 服务时机 |
|------|---------|-------------|
| **waimai**（胡闹外卖）| **v2 重建中**（DRAMA 引擎 + brain 契约）| ✅ M0 接入 |
...
包含 **5 个新字段**（三司会审沉淀）；**v2.2 增补 `food.branches`**（D1 合同补缺，详见文末）：
...
```json
{
  ...
  "food": {
    ...
    // 🆕 v2.2（玩法师提 / 锡哥授权直改）：DramaBranch 数组 = DRAMA 引擎核心输入
    // 无此字段 brain 喂不动 waimai v2 的 DRAMA 引擎（详见文末「v2.2 与外卖 v2 对齐」）
    // 完整字段形状以 whoknow-waimai/docs/GDD-v2-2026-07-24.md §9.4 为准
    // 🆕 字段级权威见 whoknow-waimai/docs/specs/DATA-STRUCTURE-v1-2026-07-24.md §3.6（冲突以该规范胜出）
    "branches": [
      {
...
- **绝不静默失败**（玩家至少知道状态）
- **温和不焦虑**（L4 文案不吓人）
- **🆕 v2.2（D3 裁定）**：水印**只进环境页脚**（首页/我的订单底部），**绝不覆盖戏精弹层 / 气泡 / 结局卡**——那些是截图爆点，要保持干净美团伪装（与 waimai v2 P2 对齐）
...
## 🆕 v2.2 增量：与胡闹外卖 v2 GDD 对齐（玩法师提 · 锡哥授权直改 · 2026-07-24）

> **背景**：锡哥推翻旧版 waimai（判定「太粗糙、不是想要的」），命玩法师基于最新原型 + 仓库既有文档（DRAMA-ENGINE-V2 / 本 api-spec / BRAND / 禁忌词清单）重做规划。重做稿 `whoknow-waimai/docs/GDD-v2-2026-07-24.md`（v2.2）。本文件据此补 3 处合同对齐，**码农虾无需重新设计，只需认下以下约定**。
...
### D2 · M0 目标 = 新 waimai v2（已直改 §服务的矩阵）
- **问题**：矩阵原写「waimai v18 上线」指旧版，与本次推翻重建不符。
- **已改**：矩阵改为「**v2 重建中（DRAMA 引擎 + brain 契约）✅ M0 接入**」。
- **裁定**：brain 的 M0 服务目标 = **新 waimai v2**（同一 §9 契约、新引擎）。旧 v1 代码（含 `/api/chaos-brain`）废弃，不迁移。

### D3 · 水印 vs 截图伪装（已直改 §P0-3 关键设计）
- **问题**：本文件 P0-3 要「永远显示水印」，waimai v2 要「8 分美团皮截图传播」——看似冲突。
- **裁定（两边都要保）**：水印**只进环境页脚**（首页/我的订单底部），**绝不覆盖戏精弹层 / 气泡 / 结局卡**。玩家截戏精高光时水印不在画面里，截图保持干净美团伪装。P0-3 已加此条。

### ⚠️ 禁忌词红线（落地前必洗稿）
- `DRAMA-ENGINE-V2.md` 自身的示例分支（上吊 / 炸弹 / 警察上门 / 暴食进 ICU / 诈尸还魂）**全是 `禁忌词清单-v1.0` 红灯**。
- 无论 brain 生成还是预制 seed，**所有话术必须过 `forbidden` 过滤（红灯 0 容忍）**；GDD §9.4 已给合规版示例供参考。

---

_v2.0 · 2026-07-24 锡哥拍板"推倒重来" · 码农虾落档_
_v2.1 增量 · 锡哥 AA 拍板 · 2026-07-24 22:08_
_v2.2 增量 · 玩法师提 / 锡哥授权直改 · 2026-07-24 22:38 · 对齐外卖 v2 GDD（D1 补 branches / D2 改 M0 目标 / D3 水印裁定）_
_基于 R4 三司会审 + 外卖 v2 重规划_