// dramaEngine.impl.ts — DRAMA 解析器 / 状态机原型（Phase 3 · A2）
// 以 ADR-001 (数据形状权威) + DATA-STRUCTURE-v1 为准。
// 运行：node --test --experimental-strip-types tests/drama-engine/dramaEngine.test.ts
//
// 范围（原型）：加载 branches[] → 条件串解析 → 权重池选支 → 四阶段 chain 流动
//   → moodDelta/delay/effect 累加 → 产出 DramaEvent[] + finalState + newFlags。
// 不做（留给 M1）：真实骰子动画、UI 渲染、localStorage 持久化、brain 拉取降级。
...
function getVar(name: string, oi: OrderInput, hist: HistoryParams, meta: MetaParams): unknown {
  ...
  if (name in meta && meta[name] !== undefined) return meta[name]
  return undefined
}
...
export function runDrama(branches: any[], orderInput: OrderInput, opts: RunOpts = {}): RunResult {
  ...
  if (pool.length > 0) {
    ...
    if (!selected) selected = pool[pool.length - 1].branch
  }
