...
**列**：组合 | bossMood | 带 | accept | cook | deliver（主线 + §3.6 骑手旁白） | complete
**说明**：
- `deliver` 为**骑手**台词 = `RIDER_LINE[slow/fast][addr]`（承接地址情境 + cook 状态）**拼接** `RIDER_ASIDE[addr][REMARK_INDEX[remark]]`（骑手环境旁白，见 §3.6）。旁白为纯环境描写、**不回声客户备注**（R4 精化）。
- 慢单（bossMood ≤ 30）= `toilet+more_spicy / toilet+less_spicy / toilet+no_cilantro / toilet+perform` 共 4 单走 `slow` 主线；其余 20 单走 `fast`。
- `home` 在 P2 已按 **neutral(55) / warm(60+)** 拆双版（回应规范原 Q3）：neutral 用「跟给自己家做一样」、warm 用「跟回自己家一样」+「碗我顺手洗了」，语气差更明显。
- 所有台词逐字中文，过红线（无真实医疗机构名/死亡/暴力/竞品；ICU 温柔中性；公厕化粪池调侃不脏话），且荒诞但自洽。

### 3.1 公厕 toilet（cold，可达 hostile / neutral）

| 组合 | bossMood | 带 | accept | cook | deliver（主线 + §3.6 骑手旁白） | complete |
|---|---|---|---|---|---|---|
...
| home+less_spicy | 55 | neutral | 家庭单？跟给自己家做一样，随便坐。少放辣？行，清淡点。 | （哼着歌）家的味道，火候我拿捏得准。辣子我手抖少抓了一把。 | 你家这栋我熟，溜达着就到了，门把手给您留着。（邻居狗今儿没骂我） | 拿好，趁热吃——再不吃咱妈要打电话查岗了。清清淡淡，养胃。 |
| home+no_cilantro | 55 | neutral | 家庭单？跟给自己家做一样，随便坐。不要香菜？又是你。 | （哼着歌）家的味道，火候我拿捏得准。香菜我一根没放，您放心。 | 你家这栋我熟，溜达着就到了，门把手给您留着。（您家香味先飘下来了） | 拿好，趁热吃——再不吃咱妈要打电话查岗了。（确定没香菜，我检查三遍了） |
...
| #5 备注回声后半程缺失 | 全表 | 每个备注在 accept 引入、cook+complete 各呼应（R4）；deliver 主线由骑手承接地址情境、后缀为纯环境旁白（§3.6），均不回声备注内容，符合「备注不改配送则不呼应」 |

### 3.6 骑手环境旁白池（P2 内容补强 · 治「骑手零变体」）

为防复玩同地址被穿帮，每个地址配 6 句**纯环境描写**旁白，按 `REMARK_INDEX[remark]`（0–5）确定性轮转（零随机）：

| 地址 | 0 more_spicy | 1 less_spicy | 2 no_cilantro | 3 no_scold | 4 perform | 5 boss_thx |
|---|---|---|---|---|---|---|
...
- **R4 精化**：旁白只描写骑手视角的环境/心情，**绝不回声客户备注内容**（如「多放辣」不会出现在旁白里）。它只是让同一地址不同订单在骑手线有微妙变化，避免玩家重复游玩时察觉骑手台词死板。
- 选取下标由备注决定（确定性映射），故同一 `(addr, remark)` 永远得到同一句旁白，S2「无随机」仍成立。
- 旁白与 RIDER_LINE 主线拼接后整体作为 `deliver` 文本（S7 契约锁逐字校验）。
